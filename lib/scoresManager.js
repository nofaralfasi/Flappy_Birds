...
var DB_USER = 'root';
var DB_PASS = '';
var DB_DATA = 'flappybirds';

var NUMBER_OF_HIGHSCORES_TO_RETRIEVE = 10;
...
* It will try to reach a DB by default (best way to store data). But if you don't have a MySQL server or if the class
...
function ScoresManager() {
    ...
    testConnection.connect(function (err) {
        if (err) {
            console.info("\n\t[ScoresManager] Can't connect user [" + DB_USER + '] to database hosted in [' + DB_HOST + ']');
            console.info("\t[ScoresManager] Players score will be store in an array, and loose when server will be off\n");
            isDbAvailable = false;
        } else {
            console.info("\n\t[ScoresManager] Database available and connected !\n");
            isDbAvailable = true;
...
ScoresManager.prototype.setPlayerHighScore = function (player) {
    ...
    if (isDbAvailable === true) {
        ...
        connection.query("SELECT `hs_score` AS `HS` FROM `highscores` WHERE `hs_player` = " + nick + ";", function (err, rows, fields) {
            ...
            if (rows.length === 0) {
                ...
                console.info("\n\t[ScoresManager] Unknown player " + nick + '. Insert him in DB :) !');
...
ScoresManager.prototype.savePlayerScore = function (player, lastScore) {
    var nick = player.getNick(),
...
ScoresManager.prototype.getHighScores = function (callback) {
    var connection,
        hsArray = null,
        nbRes,
        i, key;
    ...
    if (isDbAvailable === true) {
        ...
        connection.query("SELECT * FROM `highscores` ORDER BY `highscores`.`hs_score` DESC LIMIT 0, " + NUMBER_OF_HIGHSCORES_TO_RETRIEVE, function (err, rows, fields) {
            if (rows.affectedRows <= 0) {
                console.error('\n\t[MYSQL ERROR] Cannot retrieve highscore in DB');
            } else {
                nbRes = rows.length;
                hsArray = [];
        ...
        // Return the NUMBER_OF_HIGHSCORES_TO_RETRIEVE best scores
        hsArray = [];
        nbRes = (this._bestScore.length > NUMBER_OF_HIGHSCORES_TO_RETRIEVE) ? NUMBER_OF_HIGHSCORES_TO_RETRIEVE : this._bestScore.length;
