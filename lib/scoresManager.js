...
ScoreSystem.prototype.getHighScores = function (callback) {
    ...
    if (isDbAvailable === true) {
        ...
        connection.query("SELECT * FROM highscores ORDER BY highscores.hs_score DESC LIMIT 0, " + NUM_OF_HS_TO_RETRIEVE, function (err, rows, fields) {
        ...
        hsArray = [];
        nbRes = (this._bestScore.length > NUM_OF_HS_TO_RETRIEVE) ? NUM_OF_HS_TO_RETRIEVE : this._bestScore.length;
