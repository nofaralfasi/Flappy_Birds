...
require(['canvasPainter', 'playersManager', '../../../shared/constants'], function (canvasPainter, PlayersManager, Const) {
    ...
    function draw(currentTime, elapsedTime) {
        // If player score is > 15, night !!
        if ((_gameState === enumState.OnGame) && (_playerManager.getCurrentPlayer().getScore() === 15))
            _isNight = true;

        canvasPainter.draw(currentTime, elapsedTime, _playerManager, _pipeList, _gameState, _isNight);
    }
    ...
    function gameLoop() {
        ...
        if (_lastTime) {
            elapsedTime = now - _lastTime;
        }
        _lastTime = now;

        // Call draw with the elapsed time between the last frame and the current one
        draw(now, elapsedTime);
    }
    ...
    function loadGameRoom() {
        ...
        // Else store it in session storage if available
        else {
            if (typeof sessionStorage != 'undefined') {
    ...
    function displayRanking(score) {
        ...
        // Display high scores in a middle of the waiting time
    ...
    function infoPanel(isShow, htmlText, timeout) {
        ...
        if (_infPanelTimer != null) {
            window.clearTimeout(_infPanelTimer);
            _infPanelTimer = null;
        }
